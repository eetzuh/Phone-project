...
function clickingOnButton(letters) {
  ...
  if (currentTime - time < 900 && previous == letters) {
    ...
    if (upper == true) {
      message =
        message.slice(0, -1) +
        letters.substring(count, count + 1).toUpperCase();
    } else {
...
function eraser() {
  if (called == true) {
    ...
    document.getElementById("textual").innerHTML = document
      .getElementById("textual")
      .innerHTML.slice(
        0,
        document.getElementById("textual").innerHTML.length - 1
      );
  }
}
...
function submit() {
  if (document.getElementById("textual").innerHTML !== " ") {
    document.getElementById("submited-msg").innerHTML +=
      document.getElementById("textual").innerHTML + "<br>";
    document.getElementById("textual").innerHTML = "";
  }
}
...
function goToMenu() {
  ...
  document.getElementById("textual").innerHTML =
    textual.parentNode.removeChild(textual);
  screen = "menu";
  console.log(memory);
}
function goBack() {
  if (screen == "menu") {
    ...
    document
      .getElementById("shownScreen")
      .appendChild(document.createElement("p"))
      .setAttribute("id", "textual");
...
function call() {
  ...
  if (regexForPlus.test(document.getElementById("textual").innerHTML) == true) {
    let num = document
      ...
      .innerHTML.substring(
        1,
        document.getElementById("textual").innerHTML.length
      );
    called = true;
    return window.open(`viber://chat?number=${num}`);
  } else if (
    regexForZero.test(document.getElementById("textual").innerHTML) == true
  ) {
    let num =
      "382" +
      document
